<template>
  <div class="dc-mkt-performance-statistics">
    <vol-form
      ref="searchForm"
      :formFields="searchFormFields"
      :formRules="searchFormRules"
      :labelWidth="100"
    >
      <template #footer>
        <el-button type="primary" @click="search">查 询</el-button>
        <el-button @click="resetSearch">重 置</el-button>
        <el-button type="success" plain @click="exportData">导 出</el-button>
      </template>
    </vol-form>
    <vol-table
      ref="table"
      :columns="columns"
      :url="url"
      :pagination="pagination"
      :ck="false"
      :index="true"
    />
  </div>
</template>
<script setup>
import VolTable from '@/components/basic/VolTable.vue'
import VolForm from '@/components/basic/VolForm.vue'
import { ref, reactive, getCurrentInstance } from 'vue'
const { proxy } = getCurrentInstance()
const url = '/api/DC/DC_MktPerformanceStatistic/'
const searchFormFields = reactive({ permissionUsername: '', dateRange: '' })
const searchFormRules = [
  [
    { title: '管理员账号', field: 'permissionUsername', type: 'like' },
    { title: '日期范围', field: 'dateRange', type: 'datetime', range: true }
  ]
]
const columns = reactive([
  { field: 'date', title: '日期', type: 'string', width: 100, sort: true },
  { field: 'permissionUsername', title: '管理员账号', type: 'string', width: 120 },
  { field: 'memberNumber', title: '会员数', type: 'int', width: 80 },
  { field: 'rechargeAmount', title: '充值金额', type: 'decimal', width: 100 },
  { field: 'withdrawalAmount', title: '提现金额', type: 'decimal', width: 100 },
  { field: 'commissionAmount', title: '佣金金额', type: 'decimal', width: 100 },
  { field: 'parentCommissionAmount', title: '上级佣金金额', type: 'decimal', width: 110 },
  { field: 'giveAmount', title: '赠送金额', type: 'decimal', width: 90 },
  { field: 'signAmount', title: '签到金额', type: 'decimal', width: 90 },
  { field: 'orderNumber', title: '订单数', type: 'int', width: 80 },
  { field: 'betNumber', title: '投注数', type: 'int', width: 80 },
  { field: 'taskCompleteNumber', title: '任务完成数', type: 'int', width: 90 },
  { field: 'resetNumber', title: '重置次数', type: 'int', width: 80 }
])
const pagination = reactive({ total: 0, size: 30, sortName: 'Date' })
const table = ref(null)
const searchForm = ref(null)
const search = () => {
  let wheres = []
  if (searchFormFields.permissionUsername) wheres.push({ name: 'permissionUsername', value: searchFormFields.permissionUsername, displayType: 'like' })
  if (searchFormFields.dateRange) wheres.push({ name: 'date', value: searchFormFields.dateRange, displayType: 'range' })
  table.value.load(wheres)
}
const resetSearch = () => {
  Object.keys(searchFormFields).forEach(k => { searchFormFields[k] = '' })
  table.value.load([])
}
const exportData = () => {
  let wheres = []
  if (searchFormFields.permissionUsername) wheres.push({ name: 'permissionUsername', value: searchFormFields.permissionUsername, displayType: 'like' })
  if (searchFormFields.dateRange) wheres.push({ name: 'date', value: searchFormFields.dateRange, displayType: 'range' })
  proxy.http.download(url + 'export', { wheres }, '业绩统计.xlsx', true)
}
</script>
<style lang="less" scoped>
.dc-mkt-performance-statistics { padding: 10px; }
</style>
