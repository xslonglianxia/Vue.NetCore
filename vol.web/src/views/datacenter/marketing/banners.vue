<template>
  <div class="dc-mkt-banners">
    ...
    <vol-table
      ...
      :index="true"
    />
    <vol-box v-model="editBoxVisible" title="轮播图编辑" :width="650" :height="450">
      <vol-form
        ref="editForm"
...
const url = '/api/DC/DC_MktBanner/'
const searchFormFields = reactive({ name: '', kind: '', enabled: '' })
...
const editFormFields = reactive({ name: '', pic: '', link: '', kind: '', seq: 0, enabled: 1, remark: '' })
const editFormRules = [
  [{ title: '名称', field: 'name', required: true }, { title: '图片', field: 'pic', type: 'img' }],
  [{ title: '链接', field: 'link' }, { title: '类型', field: 'kind', type: 'select', data: [] }],
...
const columns = reactive([
  ...
  { field: 'link', title: '链接', type: 'string', width: 150 },
  { field: 'kind', title: '类型', type: 'int', width: 80, bind: { key: 'dc_mkt_banner_kind', data: [] } },
