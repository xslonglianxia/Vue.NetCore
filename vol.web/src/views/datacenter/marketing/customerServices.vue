<template>
  <div class="dc-mkt-customer-services">
    <vol-form
      ref="searchForm"
    ...
    <vol-table
      ...
      :index="true"
    />
    <vol-box v-model="editBoxVisible" title="客服编辑" :width="650" :height="400">
      <vol-form
        ref="editForm"
...
const url = '/api/DC/DC_MktCustomerService/'
const searchFormFields = reactive({ name: '', enabled: '' })
...
const editFormFields = reactive({ name: '', pic: '', link: '', seq: 0, enabled: 1, remark: '' })
const editFormRules = [
  [{ title: '名称', field: 'name', required: true }, { title: '图片', field: 'pic', type: 'img' }],
  [{ title: '链接', field: 'link' }, { title: '排序', field: 'seq', type: 'number' }],
  [{ title: '启用', field: 'enabled', type: 'select', data: [{ key: 1, value: '是' }, { key: 0, value: '否' }] }],
...
.dc-mkt-customer-services { padding: 10px; }
