<template>
  <div class="dc-mkt-trading-flows">
    <vol-form
      ref="searchForm"
...
const url = '/api/DC/DC_MktTradingFlow/'
const searchFormFields = reactive({ flowNo: '', memberUsername: '', tradingKind: '', amountRange: '', createDateRange: '' })
const searchFormRules = [
  [
    { title: '流水号', field: 'flowNo' },
    { title: '会员账号', field: 'memberUsername', type: 'like' },
    { title: '交易类型', field: 'tradingKind', type: 'select', data: [] }
  ],
  [
...
const search = () => {
  let wheres = []
  if (searchFormFields.flowNo) wheres.push({ name: 'flowNo', value: searchFormFields.flowNo })
  ...
  if (searchFormFields.tradingKind) wheres.push({ name: 'tradingKind', value: searchFormFields.tradingKind })
...
.dc-mkt-trading-flows { padding: 10px; }
