<template>
  <div class="dc-mkt-withdrawal-kinds">
    <vol-form
      ref="searchForm"
    ...
    <vol-table
      ...
      :index="true"
    />
    <vol-box v-model="editBoxVisible" title="提现类型编辑" :width="700" :height="450">
      <vol-form
        ref="editForm"
...
const url = '/api/DC/DC_MktWithdrawalKind/'
const searchFormFields = reactive({ name: '', kind: '' })
...
const editFormFields = reactive({ name: '', kind: '', icon: '', exchangeRate: 0, feePrivateKeyConfig: '', param: '', seq: 0, remark: '' })
const editFormRules = [
  [{ title: '名称', field: 'name', required: true }, { title: '类型', field: 'kind' }],
  [{ title: '图标', field: 'icon', type: 'img' }, { title: '汇率', field: 'exchangeRate', type: 'number' }],
  [{ title: '手续费私钥配置', field: 'feePrivateKeyConfig', type: 'textarea' }],
  [{ title: '参数', field: 'param', type: 'textarea' }],
  [{ title: '排序', field: 'seq', type: 'number' }, { title: '备注', field: 'remark' }]
]
const columns = reactive([
  { field: 'name', title: '名称', type: 'string', width: 120, sort: true },
  { field: 'kind', title: '类型', type: 'string', width: 80 },
  { field: 'icon', title: '图标', type: 'img', width: 80 },
  { field: 'exchangeRate', title: '汇率', type: 'decimal', width: 90 },
  { field: 'feePrivateKeyConfig', title: '手续费私钥配置', type: 'string', width: 150, hidden: true },
  { field: 'param', title: '参数', type: 'string', width: 120 },
...
.dc-mkt-withdrawal-kinds { padding: 10px; }
