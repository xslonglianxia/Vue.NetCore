<template>
  <div class="dc-mkt-categories">
    ...
    <vol-table
      ...
      :index="true"
    />
    <vol-box v-model="editBoxVisible" title="分类编辑" :width="650" :height="400">
      <vol-form
        ref="editForm"
...
const url = '/api/DC/DC_MktCategory/'
const searchFormFields = reactive({ title: '', enabled: '' })
const searchFormRules = [
  [
    { title: '分类名称', field: 'title', type: 'like' },
    { title: '启用', field: 'enabled', type: 'select', data: [{ key: 1, value: '是' }, { key: 0, value: '否' }] }
  ]
]
const editFormFields = reactive({ title: '', subtitle: '', pic: '', seq: 0, enabled: 1, remark: '' })
const editFormRules = [
  [{ title: '分类名称', field: 'title', required: true }, { title: '副标题', field: 'subtitle' }],
  [{ title: '图片', field: 'pic', type: 'img' }, { title: '排序', field: 'seq', type: 'number' }],
  [{ title: '启用', field: 'enabled', type: 'select', data: [{ key: 1, value: '是' }, { key: 0, value: '否' }] }],
  [{ title: '备注', field: 'remark', type: 'textarea', colSize: 24 }]
]
const columns = reactive([
  { field: 'title', title: '分类名称', type: 'string', width: 120, sort: true },
  { field: 'subtitle', title: '副标题', type: 'string', width: 120 },
...
.dc-mkt-categories { padding: 10px; }
