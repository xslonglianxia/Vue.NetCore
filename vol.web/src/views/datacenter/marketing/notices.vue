<template>
  <div class="dc-mkt-notices">
    ...
    <vol-table
      ...
      :index="true"
    />
    <vol-box v-model="editBoxVisible" title="通知编辑" :width="700" :height="450">
      <vol-form
        ref="editForm"
...
const editFormFields = reactive({ title: '', content: '', enabled: 1, midList: '' })
const editFormRules = [
  [{ title: '标题', field: 'title', required: true }],
  [{ title: '内容', field: 'content', type: 'textarea', colSize: 24 }],
  [{ title: '启用', field: 'enabled', type: 'select', data: [{ key: 1, value: '是' }, { key: 0, value: '否' }] }],
  [{ title: '会员ID列表', field: 'midList', type: 'textarea', colSize: 24 }]
]
const columns = reactive([
  ...
  { field: 'midList', title: '会员ID列表', type: 'string', width: 150 },
